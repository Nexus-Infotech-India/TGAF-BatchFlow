...
export function convertToBaseUOM(
  ...
  // Return quantity as-is if units are the same
  if (from === base) {
    return quantity;
  }

  if (!conversions[base] || !conversions[base][from]) {
    console.warn(`Cannot convert from ${from} to ${base}, returning original quantity`);
    return quantity; // Fallback: return original quantity instead of throwing
  }
