...
const VendorBox: React.FC<VendorBoxProps> = ({ vendor, onCreated }) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setMsg("");
    try {
      const res = await api.post(API_ROUTES.RAW.CREATE_VENDOR, form);
  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-5 w-80">
      ...
      <dl className="divide-y divide-gray-100">
        ...
        {vendor.gstin && (
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-28">GSTIN</dt>
            <dd className="text-sm text-gray-900">{vendor.gstin}</dd>
          </div>
        )}
        {vendor.bankDetails && (
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-28">Bank Details</dt>
            <dd className="text-sm text-gray-900">{vendor.bankDetails}</dd>
          </div>
        )}
        <div className="py-2 flex items-start gap-2">
