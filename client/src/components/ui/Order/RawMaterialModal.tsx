...
import { X, Search, Package, Check, ChevronRight } from "lucide-react";
...
const RawMaterialModal: React.FC<RawMaterialModalProps> = ({
    ...
    const filteredMaterials = rawMaterials.filter(
        (rm) =>
            ...
            rm.skuCode.toLowerCase().includes(searchQuery.toLowerCase()) ||
                            ...
                            {filteredMaterials.length === 0 ? (
                                ...
                                filteredMaterials.map((rm) => (
                                    <motion.button
                                        ...
                                        type="button"
                                        onClick={() => onSelect(rm)}
                                        className={`w-full text-left px-4 py-3 rounded-xl mb-1 transition-all duration-150 flex items-center gap-3 group ${selectedRawMaterialId === rm.id
                                    ...
                                    >
                                        <div
                                            className={`w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 ${selectedRawMaterialId === rm.id
                                                    ? "bg-primary text-primary-foreground"
                                                    : "bg-muted text-muted-foreground group-hover:bg-primary/10 group-hover:text-primary"
                                                }`}
                                        >
                                            {selectedRawMaterialId === rm.id ? (
                                        ...
                                        <div className="flex-1 min-w-0">
                                            ...
                                            <p className="text-xs text-muted-foreground truncate">
                                                {rm.skuCode} · {rm.category} · {rm.unitOfMeasurement}
                                            </p>
                                        </div>
