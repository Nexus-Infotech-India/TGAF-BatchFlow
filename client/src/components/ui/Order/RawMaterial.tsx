...
const RawMaterialBox: React.FC<RawMaterialBoxProps> = ({ rawMaterial, onCreated }) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await api.post(API_ROUTES.RAW.CREATE_PRODUCT, payload);
  ...
  if (showForm) {
    return (
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 w-96">
        ...
        <form onSubmit={handleCreate} className="space-y-3">
          ...
          <button
            type="submit"
            className="w-full bg-blue-600 text-white py-3 px-6 rounded-xl font-semibold text-base hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200"
            disabled={loading}
          >
            {loading ? "Creating..." : "Create Raw Material"}
          </button>
          {msg && <div className="text-center text-sm text-blue-700 mt-2">{msg}</div>}
        </form>
      </div>
    );
  }
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-200 p-6 w-96">
        ...
        <dl className="divide-y divide-gray-100">
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-36">SKU Code</dt>
            <dd className="text-sm text-gray-900">{rawMaterial.skuCode}</dd>
          </div>
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-36">Name</dt>
            <dd className="text-sm text-gray-900">{rawMaterial.name}</dd>
          </div>
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-36">Category</dt>
            <dd className="text-sm text-gray-900">{rawMaterial.category}</dd>
          </div>
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-36">Unit of Measurement</dt>
            <dd className="text-sm text-gray-900">{rawMaterial.unitOfMeasurement}</dd>
          </div>
          <div className="py-2 flex items-start gap-2">
            <dt className="text-xs font-medium text-gray-500 w-36">Min Reorder Level</dt>
