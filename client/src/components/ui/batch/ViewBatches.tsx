...
import {
  Search, Filter, Download, Plus, CheckCircle, XCircle, Clock, FileText, Eye,
  ChevronLeft, ChevronRight, AlertCircle, Calendar, Package, ChevronDown,
  RefreshCw, X, Beaker, FlaskConical, ArrowLeft, Sparkles, TrendingUp,
  Target, Award, Star, Users, Activity, BarChart3, Gauge, 
  Zap, Shield, Microscope, AlertTriangle
} from "lucide-react";
        ...
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h4 className={`font-bold ${
              ...
              {isVerified ? 'Verification Completed' : 'Verification Not Completed Yet'}
            </h4>
            <p className={`text-sm ${
              isVerified 
                ? 'text-green-700' 
        ...
        <table className="w-full border-collapse bg-white">
          ...
          <tbody>
            {parameters.map((parameter, index) => {
              ...
              const hasVerificationResult = parameter.verificationResult && parameter.verificationResult.trim() !== '';
              const hasVerificationRemark = parameter.verificationRemark && parameter.verificationRemark.trim() !== '';
              ...
              return (
                ...
                >
                  <td className="p-4 border-r border-gray-100">
                    <div>
                      <p className="font-semibold text-gray-900 mb-1">{parameter.parameter?.name}</p>
                      {parameter.parameter?.description && (
                        <p className="text-xs text-gray-500 leading-relaxed">{parameter.parameter.description}</p>
                      )}
                    </div>
                  </td>
                  ...
                  <td className="p-4 border-r border-gray-100">
                    <div className="flex items-center space-x-2">
                      <span className="font-bold text-gray-900 bg-gradient-to-r from-blue-600 to-indigo-600 bg-clip-text text-transparent">
                        {parameter.standardDefinition?.standardValue || parameter.value || 'N/A'}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="p-4 border-r border-gray-200">
                    <span className="px-2 py-1 bg-gray-100 text-gray-700 text-sm font-medium rounded-lg">
                      {parameter.standardDefinition?.unit?.symbol || parameter.unit?.symbol || '-'}
                    </span>
                  </td>
...
const BatchDetails = ({ batch, onBack }: { batch: any; onBack: () => void }) => {
  const getCategoryIcon = (categoryName: string) => {
    switch (categoryName.toLowerCase()) {
      case 'physical': return <Gauge className="text-blue-600" size={18} />;
      case 'chemical': return <Beaker className="text-emerald-600" size={18} />;
      case 'microbiological': return <Microscope className="text-purple-600" size={18} />;
      case 'nutritional': return <Shield className="text-amber-600" size={18} />;
      case 'sensory': return <Star className="text-pink-600" size={18} />;
      case 'safety': return <AlertTriangle className="text-red-600" size={18} />;
      default: return <FlaskConical className="text-gray-600" size={18} />;
    }
  };

  const getCategoryGradient = (categoryName: string) => {
    switch (categoryName.toLowerCase()) {
      case 'physical': return 'from-blue-50 to-blue-100 border-blue-200';
      case 'chemical': return 'from-emerald-50 to-emerald-100 border-emerald-200';
      case 'microbiological': return 'from-purple-50 to-purple-100 border-purple-200';
      case 'nutritional': return 'from-amber-50 to-amber-100 border-amber-200';
      case 'sensory': return 'from-pink-50 to-pink-100 border-pink-200';
   ...
   const handleExportToCOA = async () => {
    ...
    Object.entries(batch.parameterValuesByCategory as Record<string, any[]>).forEach(([category, params]) => {
      (params as any[]).forEach(param => {
        const hasValue = param.value && param.value.trim() !== '';
        const hasVerificationResult = param.verificationResult && param.verificationResult.trim() !== '';
        
        parameters.push({
          category: category,
          name: param.parameter?.name || 'Unknown Parameter',
          standardValue: param.standardDefinition?.standardValue || param.value || 'N/A',
          unit: param.standardDefinition?.unit?.symbol || param.unit?.symbol || '',
          result: hasVerificationResult ? param.verificationResult : hasValue ? param.value : 'No test data',
          remark: param.verificationRemark || (hasValue ? 'No remarks' : 'No data available')
        });
      });
    });
    ...
    >
      <div className="max-w-7xl mx-auto px-4 space-y-6">
       ...
       <motion.div
          ...
          <div className="relative bg-gradient-to-r from-blue-50 via-indigo-50 to-purple-50 p-6 border-b border-gray-100">
            ...
            <div className="relative">
              <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                ...
                <div className={`inline-flex items-center px-4 py-2 rounded-xl text-sm font-bold border shadow-md ${statusColors[batch.status as keyof typeof statusColors]}`}>
        ...
        >
          <motion.div 
            whileHover={{ y: -2, scale: 1.02, boxShadow: "0 12px 24px rgba(59, 130, 246, 0.1)" }}
          ...
          >
            ...
            <div className="relative">
              <p className="text-xs font-bold text-blue-700 mb-1">Batch Number</p>
              <p className="text-2xl font-bold text-blue-900 mb-1">{batch.batchNumber}</p>
              <div className="flex items-center">
                <Activity size={10} className="text-blue-600 mr-1" />
                <span className="text-xs text-blue-700 font-medium">Active Batch</span>
              </div>
            </div>
          </motion.div>

          <motion.div 
            whileHover={{ y: -2, scale: 1.02, boxShadow: "0 12px 24px rgba(16, 185, 129, 0.1)" }}
          ...
          >
            ...
            <div className="relative">
              <p className="text-xs font-bold text-emerald-700 mb-1">Created By</p>
              <p className="text-xl font-bold text-emerald-900 mb-1">{batch.maker?.name || "N/A"}</p>
              <div className="flex items-center">
                <Award size={10} className="text-emerald-600 mr-1" />
                <span className="text-xs text-emerald-700 font-medium">Batch Maker</span>
              </div>
            </div>
          </motion.div>

          <motion.div 
            whileHover={{ y: -2, scale: 1.02, boxShadow: "0 12px 24px rgba(139, 92, 246, 0.1)" }}
          ...
          >
            ...
            <div className="relative">
              <p className="text-xs font-bold text-purple-700 mb-1">Verified By</p>
              <p className="text-xl font-bold text-purple-900 mb-1">{batch.checker?.name || "Pending"}</p>
              <div className="flex items-center">
                <Shield size={10} className="text-purple-600 mr-1" />
                <span className="text-xs text-purple-700 font-medium">Quality Check</span>
              </div>
            </div>
          </motion.div>

          <motion.div 
            whileHover={{ y: -2, scale: 1.02, boxShadow: "0 12px 24px rgba(245, 158, 11, 0.1)" }}
          ...
          >
            ...
            <div className="relative">
              <p className={`text-xs font-bold mb-1 ${isVerified ? 'text-green-700' : 'text-amber-700'}`}>
                Verification Status
              </p>
              <p className={`text-xl font-bold mb-1 ${isVerified ? 'text-green-900' : 'text-amber-900'}`}>
                {isVerified ? 'Completed' : 'Pending'}
              </p>
              <div className="flex items-center">
                <Star size={10} className={`mr-1 ${isVerified ? 'text-green-600' : 'text-amber-600'}`} />
                <span className={`text-xs font-medium ${isVerified ? 'text-green-700' : 'text-amber-700'}`}>
          ...
          <div className="bg-gradient-to-r from-gray-50 to-blue-50 p-4 border-b border-gray-100">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-gray-600 text-sm">Complete details and specifications</p>
              </div>
            </div>
          </div>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-base font-bold text-blue-900">Product Details</h3>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-blue-700 font-medium text-sm">Product Name:</span>
                      <span className="text-blue-900 font-bold text-sm">{batch.productName}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-blue-700 font-medium text-sm">Analysis Status:</span>
                      <span className="text-blue-900 font-bold text-sm">{batch.sampleAnalysisStatus || 'N/A'}</span>
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-base font-bold text-emerald-900">Production Timeline</h3>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-emerald-700 font-medium text-sm">Production Date:</span>
                      <span className="text-emerald-900 font-bold text-sm">{formatDate(batch.dateOfProduction)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-emerald-700 font-medium text-sm">Best Before Date:</span>
                      <span className="text-emerald-900 font-bold text-sm">{formatDate(batch.bestBeforeDate)}</span>
                    </div>
                  </div>
                </motion.div>
              </div>
              ...
              <div className="space-y-4">
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-base font-bold text-purple-900">Personnel Information</h3>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-purple-700 font-medium text-sm">Created By:</span>
                      <span className="text-purple-900 font-bold text-sm">{batch.maker?.name || "N/A"}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-purple-700 font-medium text-sm">Checked By:</span>
                      <span className="text-purple-900 font-bold text-sm">{batch.checker?.name || "Not checked yet"}</span>
                    </div>
                  </div>
                </motion.div>
                ...
                >
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <h3 className="text-base font-bold text-amber-900">System Information</h3>
                  </div>
                  <div className="space-y-2">
                    <div className="flex justify-between items-center">
                      <span className="text-amber-700 font-medium text-sm">Created Date:</span>
                      <span className="text-amber-900 font-bold text-sm">{formatDate(batch.createdAt)}</span>
                    </div>
                    <div className="flex justify-between items-center">
                      <span className="text-amber-700 font-medium text-sm">Last Updated:</span>
                      <span className="text-amber-900 font-bold text-sm">{formatDate(batch.updatedAt)}</span>
                    </div>
                  </div>
                </motion.div>
              </div>
        ...
        {batch.parameterValuesByCategory && Object.keys(batch.parameterValuesByCategory).length > 0 && (
          <motion.div
            initial={{ opacity: 0, y: 20 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="text-center">
              <h2 className="text-2xl font-bold text-gray-900 mb-3">Quality Parameters Analysis</h2>
              <p className="text-gray-600 max-w-2xl mx-auto">
                Comprehensive testing results and verification status for all quality parameters
              </p>
            </div>

            {Object.entries(batch.parameterValuesByCategory as Record<string, any[]>).map(([categoryName, parameters], index) => (
              <motion.div 
                key={categoryName}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                className="bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden"
              >
                <div className={`p-6 bg-gradient-to-r ${getCategoryGradient(categoryName)} border-b border-gray-200`}>
                  <div className="flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-2xl font-bold text-gray-900 mb-1">{categoryName} Parameters</h3>
                        <p className="text-gray-700">
                          {parameters.length} parameter{parameters.length !== 1 ? 's' : ''} tested
                          {isVerified ? ' • Verification completed' : ' • Awaiting verification'}
                        </p>
                      </div>
                    </div>
                    ...
                    >
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm">test{parameters.length !== 1 ? 's' : ''}</span>
                      </div>
                    </motion.div>
                  </div>
                </div>
            ...
            <div className="bg-gradient-to-r from-purple-50 to-indigo-50 p-6 border-b border-gray-100">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">Track all changes and updates to this batch</p>
                </div>
              </div>
            </div>
                ...
                {batch.recentActivities.map((activity: any, index: number) => (
                  ...
                  >
                    <div className="flex items-start gap-4">
                      ...
                      <div className="flex-1">
                        <p className="font-semibold text-gray-900 mb-1">{activity.details}</p>
...
export default function ViewBatches() {
  ...
  const approvedBatches = batchesData?.batches?.filter((b: any) => b.status === 'APPROVED').length || 0;
  const pendingBatches = batchesData?.batches?.filter((b: any) => b.status === 'SUBMITTED').length || 0;
  const draftBatches = batchesData?.batches?.filter((b: any) => b.status === 'DRAFT').length || 0;
                  ...
                  <div className="flex items-center space-x-3 mb-1">
                    ...
                    <div>
                      ...
                      <p className="text-gray-600 text-sm mt-0.5">
                        View and manage all batch records with comprehensive filtering and export options
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="p-6 border-b border-gray-100 bg-gradient-to-r from-gray-50/50 to-blue-50/50">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
              ...
              >
                ...
                <div className="relative">
                  <p className="text-xs font-medium text-gray-600 mb-1">Total Batches</p>
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{approvedBatches}</p>
                  <div className="flex items-center mt-1">
              ...
              >
                ...
                <div className="relative">
                  ...
                  <p className="text-2xl font-bold text-gray-900">{draftBatches}</p>
                  <div className="flex items-center mt-1">
