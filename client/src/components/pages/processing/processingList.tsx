...
interface ProcessingJob {
  ...
  inputRawMaterial?: { skuCode: string };
}
...
const ProcessingList: React.FC = () => {
  ...
  const fetchProcessingJobs = async (rawMaterialId: string) => {
    try {
  ...
  const handleExpand = (expanded: boolean, record: CleaningMaterial) => {
    ...
    if (expanded) {
      ...
      fetchProcessingJobs(record.rawMaterialId);
    } else {
      setExpandedRowKeys([]);
    }
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setModal({ visible: false, job: undefined, quantity: 0, warehouseId: "", loading: false });
      fetchCleaningJobs();
      fetchProcessingJobs(modal.job.rawMaterial.id);
    } catch {
  ...
  const handleEditStatusSubmit = async () => {
    ...
    if (!editStatusModal.byProductQuantity || editStatusModal.byProductQuantity < 0) {
  ...
  const totalQuantity = cleaningJobs.reduce((sum, j) => sum + (j.netQuantity || 0), 0);
  ...
  const expandedRowRender = (record: CleaningMaterial) => {
    const jobs = processingJobs[record.rawMaterialId] || [];
    return (
      <div className="overflow-x-auto rounded-xl border border-gray-200 shadow bg-white mt-2 p-2">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody>
            {jobs.length === 0 && (
              <tr>
                <td colSpan={7} className="px-4 py-2 text-center text-gray-400 italic">
  ...
  return (
    <motion.div className="min-h-screen">
      ...
      <Modal
        ...
        title={
          <div>
            <span className="text-lg font-semibold text-blue-700">Initiate Processing</span>
        ...
        onCancel={() => setModal({ visible: false, job: undefined, quantity: 0, warehouseId: "", loading: false })}
        onOk={handleSubmit}
      ...
      >
        <div className="mb-3">
          <div className="text-xs text-gray-500 mb-1">By-Product/Semi-Processed Quantity</div>
          <Input
            type="number"
            min={0}
