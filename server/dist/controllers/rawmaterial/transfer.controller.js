...
class TransferController {
            ...
            if (direction === 'SFG_TO_PRODUCTION') {
                const transfer = await prisma.$transaction(async (tx) => {
                    ...
                    for (const line of lines) {
                        ...
                        const sourceLine = sourceTransfer.lines.find((sl) => sl.lineType === 'SFG' && ((sl.rawMaterialId && sl.rawMaterialId === sourceRmId) ||
                    ...
                    const sfgCount = lineCreates.filter((l) => l.lineType === 'SFG').length;
                    const pkgCount = lineCreates.filter((l) => l.lineType === 'PACKAGING_MATERIAL').length;
     ...
     */
    static async createOutboundTransfer(req, res) {
        try {
            ...
            const updated = await prisma.$transaction(async (tx) => {
                ...
                if (transfer.direction === 'SFG_TO_PRODUCTION') {
                    for (const line of transfer.lines) {
                        ...
                        if (line.lineType !== 'SFG' || !line.batchNumber)
                            continue;
                        ...
                        if (sourceTransfer) {
                            ...
                            const sourceLine = sourceTransfer.lines.find((sl) => sl.lineType === 'SFG' && ((sl.rawMaterialId && sl.rawMaterialId === line.rawMaterialId) ||
     ...
     */
    static async getSfgWarehouseStock(req, res) {
        try {
            ...
            for (const c of pastConsumptions) {
                if (!c.batchNumber)
                    continue;
                if (!fgConsumedMap[c.batchNumber])
                    fgConsumedMap[c.batchNumber] = {};
                if (!fgConsumedMap[c.batchNumber][c.rawMaterialId])
                    fgConsumedMap[c.batchNumber][c.rawMaterialId] = 0;
            ...
            const dispatchBagsMap = {};
            for (const dl of allSfgDispatchLines) {
                if (!dl.batchNumber)
                    continue;
                const rmId = dl.rawMaterialId || '';
                if (!dispatchBagsMap[dl.batchNumber])
                    dispatchBagsMap[dl.batchNumber] = {};
                if (!dispatchBagsMap[dl.batchNumber][rmId])
                    dispatchBagsMap[dl.batchNumber][rmId] = 0;
            ...
            for (const transfer of outboundTransfers) {
                for (const line of transfer.lines) {
                    ...
                    const availableQty = Math.max(0, Math.round((line.quantity - fgConsumed - transferredQty) * 1000) / 1000);
     ...
     */
    static async getOutboundStockDetails(req, res) {
        try {
            const outboundTransfers = await prisma.materialTransfer.findMany({
                where: {
                    direction: 'OUTBOUND_FROM_GRINDING',
     ...
     */
    static async getPackagingStock(req, res) {
        try {
