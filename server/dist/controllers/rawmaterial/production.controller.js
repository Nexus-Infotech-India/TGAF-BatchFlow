            ...
            for (const item of bom.items) {
                ...
                if (matchingDispatches.length > 0) {
                    ...
                    availableBatches = matchingDispatches
                        ...
                        const storedUnit = d.lots?.[0]?.cleaningLot?.cleanedQuantityUnit || 'KG';
        ...
        try {
            ...
            for (const c of consumptions) {
                ...
                if (c.sourceType === 'BATCH' && c.batchNumber) {
                    ...
                    const storedUnit = dispatch.lots?.[0]?.cleaningLot?.cleanedQuantityUnit || 'KG';
                ...
                for (const c of consumptions) {
                    ...
                    if (c.sourceType === 'BATCH' && c.batchNumber) {
                        ...
                        if (dispatchWithMaterial) {
                            // totalQuantity is stored in the lot's cleanedQuantityUnit (e.g. KG)
                            const storedUnit = dispatchWithMaterial.lots?.[0]?.cleaningLot?.cleanedQuantityUnit || 'KG';
