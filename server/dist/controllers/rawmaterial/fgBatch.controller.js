...
exports.FGBatchController = void 0;
...
class FGBatchController {
     ...
     */
    static async getFGBOMItems(req, res) {
        try {
            ...
            const prodUnit = productionUnit || bom.unitOfMeasurement;
            ...
            if (locationId) {
                whereClause.toLocationId = locationId;
            }
            ...
            for (const c of pastConsumptions) {
                if (!c.batchNumber)
                    continue;
                if (!consumedMap[c.batchNumber])
                    consumedMap[c.batchNumber] = {};
                if (!consumedMap[c.batchNumber][c.rawMaterialId])
                    consumedMap[c.batchNumber][c.rawMaterialId] = 0;
            ...
            for (const bomItem of bom.items) {
                ...
                if (isSFG) {
                    ...
                    for (const transfer of acceptedTransfers) {
                        const sfgLines = transfer.lines.filter((line) => line.lineType === 'SFG' && (line.rawMaterialId === rm.id || line.productName === rm.name));
                    ...
                    let pkgLocationStockUnit = null;
                    for (const transfer of acceptedTransfers) {
                        const pkgLines = transfer.lines.filter((line) => line.lineType === 'PACKAGING_MATERIAL' && (line.rawMaterialId === rm.id || line.productName === rm.name));
                        ...
                        if (remaining > 0) {
                            ...
                            if (!pkgLocationStockUnit)
                                pkgLocationStockUnit = lineUnit;
     ...
     */
    static async getFGBOMs(req, res) {
        try {
     ...
     */
    static async createFGBatch(req, res) {
        try {
            const { bomId, productionQty, productionUnit, notes, consumptions, // Array of { rawMaterialId, actualQuantity, unit, sourceType, batchNumber, dispatchId }
             } = req.body;
            ...
            for (const c of consumptions) {
                ...
                if (c.sourceType === 'SFG_BATCH' && c.batchNumber) {
                    ...
                    const sfgLines = transfer.lines.filter((l) => l.lineType === 'SFG' && (l.rawMaterialId === c.rawMaterialId || l.productName === c.rawMaterialName));
                    if (sfgLines.length === 0) {
                    ...
                    const pkgLines = transfer.lines.filter((l) => l.lineType === 'PACKAGING_MATERIAL' && (l.rawMaterialId === c.rawMaterialId || l.productName === c.rawMaterialName));
                    if (pkgLines.length === 0) {
     ...
     */
    static async getFGBatches(req, res) {
        try {
     ...
     */
    static async getFGBatchById(req, res) {
        try {
            const { id } = req.params;
     ...
     * Accepts an FG Batch.
     */
    static async acceptFGBatch(req, res) {
        try {
            const { id } = req.params;
     ...
     * Rejects an FG Batch.
     */
    static async rejectFGBatch(req, res) {
        try {
            const { id } = req.params;
...
exports.FGBatchController = FGBatchController;
